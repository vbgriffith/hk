...
const UI = (() => {
  ...
  function onForgeComplete(campaign) {
    ...
    setTimeout(function() { drawCampaignMaps(campaign); }, 300);
  }

  function drawCampaignMaps(campaign) {
    if (typeof MapExtensions === 'undefined') return;
    ...
    if (campaign && campaign.acts) {
      campaign.acts.forEach(function(act) {
        ...
        if (canvas) {
          canvas.width  = Math.min(canvas.parentElement ? canvas.parentElement.offsetWidth - 12 : 820, 900);
          canvas.height = Math.round(canvas.width * 0.46);
          MapExtensions.generateEnvironmentMap(canvas, act, campaign);
        }
      });
    }
    // Social room maps (one per influence scene, identified by data attrs)
    document.querySelectorAll('.soc-map-canvas').forEach(function(canvas) {
      canvas.width  = Math.min(canvas.parentElement ? canvas.parentElement.offsetWidth - 12 : 600, 700);
