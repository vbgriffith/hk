...
5. ui/ (Terminal → CadenceOS → FileManager → HUD)
6. websites/ (all site definitions)
7. systems/ (BrowserEngine → DialogueEngine → PuzzleManager → CorruptionTracker)
...
- Layer 1 (PILGRIM): world renders but Oswin's dialogue tree is visual-only — Phase 3 will complete this
- Layer 2 (Workshop): file tree is decorative only — Phase 4 will make it fully interactive
- Layer 3 (Halverstrom): Cartographer walks but patience puzzle ending not yet fully scripted
- Layer 4 (Substrate): ESC escape works but fragments are read-only
- No audio — intentional for now; Phase 5 will add procedural audio via Web Audio API
