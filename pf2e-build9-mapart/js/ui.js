...
const UI = (() => {
  ...
  function onForgeComplete(campaign) {
    ...
    // Draw environment and social room maps after DOM settles
    setTimeout(function() { drawCampaignMaps(campaign); }, 500);
  }

  function drawCampaignMaps(campaign) {
    var Renderer = (typeof MapArt !== 'undefined') ? MapArt : (typeof MapExtensions !== 'undefined' ? MapExtensions : null);
    if (!Renderer) { console.warn('No map renderer loaded (MapArt or MapExtensions)'); return; }
    // Environment maps (one per act)
    if (campaign && campaign.acts) {
      campaign.acts.forEach(function(act) {
        const canvas = document.getElementById('envCanvas-act-' + act.number);
        if (!canvas) { console.warn('No env canvas for act', act.number); return; }
        // Use parent width if available and non-zero, otherwise use a safe default
        const parentW = canvas.parentElement ? canvas.parentElement.clientWidth : 0;
        canvas.width  = parentW > 100 ? parentW - 12 : 820;
        canvas.height = Math.round(canvas.width * 0.46);
        Renderer.generateEnvironmentMap(canvas, act, campaign);
      });
    }
    // Social room maps
    document.querySelectorAll('.soc-map-canvas').forEach(function(canvas) {
      const parentW = canvas.parentElement ? canvas.parentElement.clientWidth : 0;
      canvas.width  = parentW > 100 ? parentW - 12 : 600;
      canvas.height = Math.round(canvas.width * 0.53);
      var encounter = {
        npcName:  canvas.dataset.npc || '',
        template: canvas.dataset.template || '',
      };
      Renderer.generateSocialRoomMap(canvas, encounter, campaign);
    });
    console.log('drawCampaignMaps complete — env:', campaign.acts ? campaign.acts.length : 0, 'social:', document.querySelectorAll('.soc-map-canvas').length);
  }
