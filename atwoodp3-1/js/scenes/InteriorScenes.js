...
class StudyScene extends BaseScene {
  ...
  create() {
    const { W, H } = this.baseCreate();
    this.sceneId = 'study';
    gameState.setFlag('study_accessed', true);
    this.buildRoom(W, H);
    this.buildHotspots(W, H);
...
// ── ACT 4 CONFRONTATION HOTSPOT INJECTOR ─────────────────────
// Adds a "Confront Nathaniel" interaction to FoyerScene in Act 4
// Called from sceneManager.checkLocationEvents

function injectConfrontationHotspot(scene, W, H) {
  if (gameState.currentAct < 4) return;
  if (gameState.getFlag('nathaniel_confessed')) return;
  if (gameState.getFlag('path_chosen')) return;

  const g = scene.add.graphics();
  // Nathaniel standing at staircase in Act 4
  g.fillStyle(0x101820, 0.9); g.fillRect(W * 0.64, H * 0.42, 20, H * 0.22);
  g.fillEllipse(W * 0.65, H * 0.38, 20, 24);

  // Pulsing indicator
  scene.addHotspot(W * 0.6, H * 0.36, W * 0.2, H * 0.3, {
    id: 'confront_nathaniel_trigger',
    label: 'Confront Nathaniel',
    onActivate: () => {
