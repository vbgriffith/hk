...
// ── Phase 2 Extensions ──────────────────────────────────────
GameState.prototype.getClueList = function() {
  return Array.from(this.clues).map(id => STORY.clues[id]).filter(Boolean);
};

GameState.prototype.hasClue = function(id) {
...
GameState.prototype.getSummary = function() {
  return {
    act: this.currentAct,
...
GameState.prototype.checkActCompletion = function() {
  const act = STORY.acts[`act${this.currentAct}`];
  if (!act || !act.completionConditions) return false;
  return act.completionConditions.every(c => this.checkCondition(c));
};

GameState.prototype.advanceAct = function() {
  this.currentAct = Math.min(5, this.currentAct + 1);
  return this.currentAct;
};
