...
class GameScene extends Phaser.Scene {
  ...
  _clearRoom() {
    ...
    if (this._dialogue?.isVisible) {
      this._dialogue._visible = false;   // silent hide — no callback, no physics.resume
  ...
  _buildSaveData() {
    ...
    if (!snap.flags         || typeof snap.flags !== 'object')  snap.flags = Object.assign({}, this._save.flags);
    if (!snap.abilities     || typeof snap.abilities !== 'object') snap.abilities = Object.assign({}, this._save.abilities);
    if (!Array.isArray(snap.itemsCollected)) snap.itemsCollected = [...(this._save.itemsCollected ?? [])];
    if (!Array.isArray(snap.visitedRooms))   snap.visitedRooms   = [...(this._save.visitedRooms   ?? [])];
    if (!Array.isArray(snap.ownedCharms))    snap.ownedCharms    = [...(this._save.ownedCharms    ?? [])];
    if (!Array.isArray(snap.charms))         snap.charms         = [...(this._save.charms         ?? [])];
    return snap;
  }
