...
class NPC extends Entity {
  ...
  _getDialogueKeys() {
    ...
    // Flat array format: treat whole array as one conversation
    if (Array.isArray(def)) return ['_root'];
  ...
  _getDialogueLines() {
    ...
    // Flat array: the whole thing is the lines
    ...
    // Each value should be an array of line objects; if it's a single object wrap it
...
class Shade extends Entity {
  ...
  _collect(player) {
    ...
    // Clear shade from save so it doesn't respawn
    if (this.scene._save) this.scene._save.shade = null;
