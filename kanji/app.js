...
function openKanjiModal(kanjiChar) {
  ...
  document.getElementById('modal-info').innerHTML = `
    ...
    <div class="info-block"><label>JLPT Level</label><span class="tag ${k.level.toLowerCase()}" style="font-size:.85rem">${k.level}</span></div>
  `;
  ...
  document.getElementById('modal-examples').innerHTML = `
    <h4>Vocabulary using 「${k.kanji}」</h4>
...
function setWriteKanji(idx) {
  ...
  document.getElementById('write-kanji-info').innerHTML = `
    ...
    <span class="tag ${k.level.toLowerCase()}" style="margin-top:.4rem;display:inline-block">${k.level}</span>
    <span class="tag">${k.category}</span>
  `;
  document.getElementById('write-counter').textContent = `${state.writeIndex+1} / ${writableKanji.length}`;
  clearCanvas();
}

function prevWriteKanji() { setWriteKanji(state.writeIndex - 1); }
function nextWriteKanji() { setWriteKanji(state.writeIndex + 1); }
function randomWriteKanji() { setWriteKanji(Math.floor(Math.random()*writableKanji.length)); }
