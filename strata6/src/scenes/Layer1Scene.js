...
class Layer1Scene extends Phaser.Scene {
  ...
  create() {
    ...
    // Ending triggers — Oswin's post_shutdown tree emits this
    EventBus.on('ending:opendoor', () => {
      this.time.delayedCall(800, () => {
        this.scene.start('EndingScene');
      });
    }, this);

    // Cartographer day-40 reveal emits this from Layer3Scene
    EventBus.on('ending:cartographer', () => {
      // cartographer ending routes through Layer1 if player is here,
      // but EndingScene reads the flag — fire from wherever we are
      if (StateManager.get('currentLayer') === 1) {
        this.time.delayedCall(1200, () => this.scene.start('EndingScene'));
      }
    }, this);
