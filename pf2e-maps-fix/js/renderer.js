...
const Renderer = (() => {
  ...
  function renderInfluenceScene(scene, act) {
    ...
    return '<div class="soc-scene">' +
      ...
      '<div class="soc-scene-body">' +
        ...
        // Social room map
        (typeof MapExtensions !== 'undefined' ? (function() {
          var scId = 'socCanvas-' + (scene.templateName||'x').replace(/[^a-z0-9]/gi,'-').toLowerCase().slice(0,20);
          return '<div class="soc-map-container"><div class="soc-map-label">🏛 Scene Map: ' + esc(scene.templateName||'Social Scene') + '</div><canvas id="' + scId + '" class="soc-map-canvas" width="600" height="320" data-npc="' + esc(scene.npc&&scene.npc.name||'') + '" data-template="' + esc(scene.templateName||'') + '"></canvas></div>';
        })() : '') +
  ...
  function renderActs(c) {
    var actHTML = c.acts.map(function(act) {
      ...
      var envCanvasId = 'envCanvas-act-' + act.number;
      var envCanvas = '<div class="env-map-container"><div class="env-map-label">🌍 Environment: ' + esc(act.location||'Unknown') + '</div><canvas id="' + envCanvasId + '" class="env-map-canvas" width="820" height="380"></canvas></div>';
      // Room flavor text
      var flavorHTML = '';
      if (typeof MapExtensions !== 'undefined') {
        var fBlocks = [];
        var rts = ['entry','combat','treasure','ritual','boss'];
        var themeName = c.base && c.base.theme ? c.base.theme : 'classic';
        rts.forEach(function(rt) {
          var fl = MapExtensions.getRoomFlavor(rt, themeName);
          if (fl) fBlocks.push('<div class="room-flavor-block"><span class="rfb-type">' + rt.charAt(0).toUpperCase()+rt.slice(1) + ' Room</span><p class="rfb-text">' + esc(fl) + '</p></div>');
        });
        if (fBlocks.length) flavorHTML = '<details class="flavor-details"><summary class="flavor-summary">📝 Location Descriptions (' + fBlocks.length + ')</summary><div class="flavor-grid">' + fBlocks.join('') + '</div></details>';
      }
  ...
  function renderRewards(c) {
    if (!c.rewards || !c.rewards.length) return '';
    var hasHoards = c.rewards[0] && c.rewards[0].hoard;
    if (!hasHoards) {
      // Legacy simple render
      var rows = c.rewards.map(function(r) {
        return '<tr><td><strong>Act ' + r.act + '</strong></td><td>' + esc(r.items&&r.items[0]||'--') + '</td><td>' + esc(r.items&&r.items[1]||'--') + '</td></tr>';
      }).join('');
    ...
    c.rewards.forEach(function(r) {
      ...
      html += '<div class="loot-act-block">';
      html += '<div class="loot-act-header"><span class="loot-act-num">ACT ' + r.act + '</span>';
      html += '<span class="loot-act-lvl">Lvl ' + (h.actLevel||'?') + ' Hoard</span>';
      html += '<span class="loot-total-val">' + fmtGp(h.totalValue) + ' gp total</span></div>';

      // Coin parcels
      if (h.coinParcels && h.coinParcels.length) {
        html += '<div class="loot-section-head">💰 Currency (split across encounters)</div>';
        html += '<div class="loot-coins">';
        h.coinParcels.forEach(function(p, i) {
          html += '<div class="loot-coin-parcel"><span class="coin-enc">Enc ' + (i+1) + '</span>';
          html += '<span class="coin-gp">' + fmtGp(p.gp) + ' gp</span>';
          if (p.sp) html += '<span class="coin-sp">' + p.sp + ' sp</span>';
          html += '</div>';
        });
        html += '</div>';
      }
      ...
      // Permanent items
      if (h.permanentItems && h.permanentItems.length) {
        html += '<div class="loot-section-head">⚔ Permanent Items</div>';
        h.permanentItems.forEach(function(item, idx) {
          var find = h.itemFinds && h.itemFinds[idx] ? h.itemFinds[idx] : '';
          html += '<div class="loot-item perm">';
          html += '<div class="loot-item-row">';
          html += '<span class="loot-item-name">' + esc(item.name) + '</span>';
          html += '<span class="loot-item-lvl">Item ' + item.level + '</span>';
          html += '<span class="loot-rarity rarity-' + (item.rarity||'common') + '">' + esc(item.rarity||'common') + '</span>';
          html += '<span class="loot-price">' + esc(item.price||'') + '</span>';
          html += '</div>';
          if (item.traits && item.traits.length) {
            html += '<div class="loot-traits">' + item.traits.map(function(t){ return '<span class="trait-pip">' + esc(t) + '</span>'; }).join('') + '</div>';
          }
          html += '<div class="loot-item-desc">' + esc(item.desc||'') + '</div>';
          if (find) html += '<div class="loot-find-note">📍 ' + esc(find) + '</div>';
          html += '</div>';
        });
      }
      ...
      if (h.themeBonus) {
        var tb = h.themeBonus;
        html += '<div class="loot-section-head">✦ Theme Bonus Item</div>';
        html += '<div class="loot-item theme-bonus">';
        html += '<div class="loot-item-row">';
        html += '<span class="loot-item-name">' + esc(tb.name) + '</span>';
        html += '<span class="loot-item-lvl">Item ' + tb.level + '</span>';
        html += '<span class="loot-rarity rarity-' + (tb.rarity||'uncommon') + '">' + esc(tb.rarity||'uncommon') + '</span>';
        html += '<span class="loot-price">' + esc(tb.price||'') + '</span>';
        html += '</div>';
        html += '<div class="loot-item-desc">' + esc(tb.desc||'') + '</div>';
        html += '</div>';
      }

      // Consumables
      if (h.consumables && h.consumables.length) {
        html += '<div class="loot-section-head">🧪 Consumables</div>';
        html += '<div class="loot-consumables">';
        h.consumables.forEach(function(con) {
          html += '<div class="loot-con-item">';
          html += '<span class="con-name">' + esc(con.name) + '</span>';
          html += '<span class="con-type ' + (con.type||'potion') + '">' + esc(con.type||'potion') + '</span>';
          html += '<span class="con-desc">' + esc(con.desc||'') + '</span>';
          html += '<span class="con-price">' + esc(con.price||'') + '</span>';
          html += '</div>';
        });
        html += '</div>';
      }

      html += '</div>'; // .loot-act-block
    });

    return section('💎','Treasure &amp; Rewards',
      html +
      '<p class="text-sm text-muted mt-sm">Hoards follow PF2e GM Core Treasure by Level (Table 10-9). Adjust item levels ±2 as needed. Rarity: <em>uncommon</em> items need GM permission; <em>rare</em> items are special finds.</p>'
    );
  }
  ...
  // ── Random Tables renderer ─────────────────────────────────────────────────
  function renderRandomTables(c) {
    if (!c.sessionTables) return '';
    var t = c.sessionTables;

    var html = '';

    // Urban events
    if (t.urban && t.urban.length) {
      html += '<details class="rt-details" open><summary class="rt-summary">🏙 Urban Events (d' + t.urban.length + ')</summary><div class="rt-table-wrap">';
      html += '<table class="rt-table"><tbody>';
      t.urban.forEach(function(ev, i) {
        html += '<tr><td class="rt-num">' + (i+1) + '</td><td>' + esc(ev) + '</td></tr>';
      });
      html += '</tbody></table></div></details>';
    }

    // Wilderness
    if (t.wilderness && t.wilderness.length) {
      html += '<details class="rt-details"><summary class="rt-summary">🌲 Wilderness Events (d' + t.wilderness.length + ')</summary><div class="rt-table-wrap">';
      html += '<table class="rt-table"><tbody>';
      t.wilderness.forEach(function(ev, i) {
        html += '<tr><td class="rt-num">' + (i+1) + '</td><td>' + esc(ev) + '</td></tr>';
      });
      html += '</tbody></table></div></details>';
    }
    ...
    // Tavern hooks
    if (t.tavern && t.tavern.length) {
      html += '<details class="rt-details"><summary class="rt-summary">🍺 Tavern Hooks (d' + t.tavern.length + ')</summary><div class="rt-table-wrap">';
      html += '<table class="rt-table"><tbody>';
      t.tavern.forEach(function(ev, i) {
        html += '<tr><td class="rt-num">' + (i+1) + '</td><td>' + esc(ev) + '</td></tr>';
      });
      html += '</tbody></table></div></details>';
    }
    ...
    // Rumors
    if (t.rumors && t.rumors.length) {
      html += '<details class="rt-details"><summary class="rt-summary">📜 Rumors (d' + t.rumors.length + ', with truth)</summary><div class="rt-table-wrap">';
      html += '<table class="rt-table rt-rumors"><thead><tr><th>#</th><th>Rumor</th><th>Truth</th></tr></thead><tbody>';
      t.rumors.forEach(function(r, i) {
        var truthClass = r.truth === 'true' ? 'truth-true' : r.truth === 'false' ? 'truth-false' : 'truth-partial';
        var truthLabel = r.truth === 'true' ? 'True' : r.truth === 'false' ? 'False' : r.truth === 'mostly_true' ? 'Mostly True' : 'Exaggerated';
        html += '<tr><td class="rt-num">' + (i+1) + '</td><td>' + esc(r.rumor) + '</td>';
        html += '<td><span class="truth-badge ' + truthClass + '">' + truthLabel + '</span></td></tr>';
      });
      html += '</tbody></table></div></details>';
    }

    // Mission seeds
    if (t.missionSeeds && t.missionSeeds.length) {
      html += '<details class="rt-details"><summary class="rt-summary">🎯 Off-Script Mission Seeds (d' + t.missionSeeds.length + ')</summary><div class="rt-table-wrap">';
      html += '<table class="rt-table"><tbody>';
      t.missionSeeds.forEach(function(s, i) {
        html += '<tr><td class="rt-num">' + (i+1) + '</td><td>' + esc(s) + '</td></tr>';
      });
      html += '</tbody></table></div></details>';
    }
    ...
    if (t.weather && t.weather.length) {
      html += '<details class="rt-details"><summary class="rt-summary">🌦 Weather Table (d' + t.weather.length + ')</summary><div class="rt-table-wrap">';
      html += '<table class="rt-table"><tbody>';
      t.weather.forEach(function(w, i) {
        html += '<tr><td class="rt-num">' + (i+1) + '</td><td>' + esc(w) + '</td></tr>';
      });
      html += '</tbody></table></div></details>';
    }

    return section('🎲','Random Tables',
      '<p class="text-sm text-muted" style="margin-bottom:0.75rem;">Campaign-specific tables for urban events, wilderness encounters, tavern hooks, rumors, and off-script seeds. Roll or pick — these are tuned to your campaign\'s theme.</p>' +
      html
    );
  }
