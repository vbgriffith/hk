...
class InventoryScreen {
  ...
  _build() {
    ...
    this._descStatus = this.scene.add.text(W / 2, H - 15, '', {
      fontFamily: 'Cinzel', fontSize: 6, color: '#5ae3e3',
    }).setScrollFactor(0).setDepth(C.LAYER_UI + 12).setOrigin(0.5).setVisible(false);

    this._hint = this.scene.add.text(W / 2, H - 4, '[Z/X] Equip/Unequip  [I/ESC] Close', {
      fontFamily: 'Cinzel', fontSize: 6, color: '#444444',
    }).setScrollFactor(0).setDepth(C.LAYER_UI + 11).setOrigin(0.5).setVisible(false);
  }
  ...
  show() {
    ...
    // Show all charms — owned ones can be equipped, unowned are greyed out
    this._allCharms = CHARM_DEFS.map(c => c.id);
    this._ownedCharms = new Set(this.scene._save?.ownedCharms ?? []);
  ...
  _redraw() {
    ...
    const owned = this._ownedCharms ?? new Set(this.scene._save?.ownedCharms ?? []);
    ...
    for (let i = 0; i < this._allCharms.length; i++) {
      ...
      const eq  = cs?.isEquipped(id);
      const has = owned.has(id);

      // Background cell
      g.fillStyle(sel ? 0x2a2a4a : has ? 0x111122 : 0x0a0a0f);
      g.fillRoundedRect(cx, cy, cell - 2, cell - 2, 2);
      g.lineStyle(sel || eq ? 1.5 : 0.5,
        sel ? 0xffffff : eq ? 0x5ae3e3 : has ? 0x333355 : 0x1a1a22);
      g.strokeRoundedRect(cx, cy, cell - 2, cell - 2, 2);

      if (has) {
        this._drawCharmIcon(g, cx + 3, cy + 3, cell - 8, id, eq);
      } else {
        // Unowned — show dim question mark silhouette
        g.fillStyle(0x222233, 0.6);
        g.fillCircle(cx + cell/2 - 1, cy + cell/2 - 1, 4);
      }
    }

    const selDef = CHARM_BY_ID[this._allCharms[this._selIdx]];
    const selOwned = owned.has(this._allCharms[this._selIdx]);
    if (selDef) {
      this._descTitle.setText(selDef.name);
      this._descBody.setText(selOwned ? selDef.desc : '???');
      if (!selOwned) {
        this._descStatus.setText('Not yet found').setColor('#555577');
      } else {
      ...
      this._descBody.setText('Collect charms to fill this space.');
  ...
  _getDisplayObjects() {
    return [this._bg, this._title, this._notchText, this._charmGraphics,
            this._descPanel, this._descTitle, this._descBody, this._descStatus, this._hint]
      .filter(Boolean);
  }
}
