...
const Terminal = (function () {
  ...
  function processCommand(raw) {
    ...
    // Zip password attempt
    if (raw.startsWith('password:') || raw.startsWith('PASSWORD:')) {
      const pass = raw.split(':')[1]?.trim() || '';
      const result = PuzzleManager.attempt('zip_password', pass);
      if (result.success) {
        return `[correct]\n[archive unlocked]\n[type: open PILGRIM_backup.zip to launch]`;
      }
      return `[incorrect password]\n[attempts: ${result.attempts}]${result.attempts > 3 ? '\n\nhint: what did Callum name his dog?' : ''}`;
    }
