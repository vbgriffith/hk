...
(function (root) {
  ...
  const STYLES = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #insp-body { display: grid; grid-template-columns: 1fr 3px 260px; flex: 1; overflow: hidden; }
    #dom-pane {
      overflow: auto; padding: 4px 0;
      scrollbar-width: thin; scrollbar-color: var(--border) transparent;
      font-family: var(--mono); font-size: 12px; line-height: 1.65;
    }
    #insp-divider { background: var(--border); cursor: col-resize; }
    #insp-divider:hover { background: var(--accent); }
    #styles-split { display: flex; flex-direction: column; overflow: hidden; border-left: 1px solid var(--border); }
    #computed-pane { flex: 1; min-height: 0; border-bottom: 1px solid var(--border); display: flex; flex-direction: column; overflow: hidden; }
    #style-rows { flex: 1; min-height: 0; overflow-y: auto; scrollbar-width: thin; scrollbar-color: var(--border) transparent; }
    #boxmodel-pane { flex-shrink: 0; overflow: hidden; transition: max-height 0.2s ease; }
    #boxmodel-pane.collapsed #box-diagram { display: none; }
    #boxmodel-hdr:hover { background: var(--bg3); }
    #boxmodel-hdr:hover #boxmodel-arrow { color: var(--text1); }
    ...
    .style-row:hover .sr-del { opacity: 1; }
    .sp { color: var(--attr); flex-shrink: 0; min-width: 130px; cursor: text; }
    .sp:focus { outline: 1px solid var(--accent); border-radius: 2px; color: var(--text0); background: var(--bg3); padding: 0 3px; }
    .sv { color: var(--val); word-break: break-all; cursor: text; flex: 1; }
    .sv:focus { outline: 1px solid var(--accent); border-radius: 2px; color: var(--text0); background: var(--bg3); padding: 0 3px; }
    .sv.swatch::before {
    ...
    .sv.invalid { color: var(--err-fg); text-decoration: underline wavy var(--err-fg); }
    .sr-del {
      opacity: 0; color: var(--text3); cursor: pointer; font-size: 13px; line-height: 1;
      padding: 0 2px; flex-shrink: 0; transition: color 0.1s, opacity 0.1s;
      background: none; border: none; font-family: inherit;
    }
    .sr-del:hover { color: var(--err-fg); }
    .sr-overridden .sv { text-decoration: line-through; opacity: 0.45; }
    ...
    #add-prop, #add-val {
      ...
      border-radius: var(--r); color: var(--text0);
      font-family: var(--mono); font-size: 11px; padding: 3px 6px;
      outline: none; transition: border-color 0.15s;
    }
    #add-prop { width: 110px; }
    #add-prop:focus, #add-val:focus { border-color: var(--accent); }
    #add-val { flex: 1; }
    #add-btn {
      background: var(--accent); border: none; border-radius: var(--r);
      color: #fff; cursor: pointer; font-size: 11px; font-family: var(--ui);
      padding: 3px 9px; font-weight: 500; flex-shrink: 0; transition: background 0.1s;
    }
    #add-btn:hover { background: #006ee0; }
    #style-filter {
    ...
    #style-filter:focus { border-color: var(--accent); }

    /* Expandable object tree in console */
    .ot { display: inline; }
    .ot-toggle {
    ...
    .ot-toggle.o { transform: rotate(90deg); }
    .ot-preview { cursor: pointer; color: inherit; }
    .ot-preview:hover .vk, .ot-preview:hover .vp { opacity: 0.8; }
    .ot-children {
      display: none; padding-left: 14px;
      border-left: 1px solid var(--border);
      margin: 2px 0 2px 6px;
    }
    .ot-children.o { display: block; }
    .ot-row { display: block; line-height: 1.6; }
    .ot-key { color: var(--attr); }
    .ot-colon { color: var(--text3); }
    .ot-idx { color: var(--num); }
      ...
      <div id="insp-body">
        ...
        <div id="styles-split">
          <div id="computed-pane">
            <div class="pane-hdr" style="display:flex;align-items:center;gap:6px;padding-right:6px">
              <span style="flex:1">Computed Styles</span>
            </div>
            <div style="padding:5px 8px;border-bottom:1px solid var(--border);background:var(--bg2)">
              <input id="style-filter" type="text" placeholder="Filter properties…" autocomplete="off" />
            </div>
            <div id="style-rows"><div class="muted">Select an element</div></div>
            <div id="add-rule-row">
              <input id="add-prop" type="text" placeholder="property" autocomplete="off" spellcheck="false" />
              <span style="color:var(--text3);font-family:var(--mono);font-size:11px">:</span>
              <input id="add-val" type="text" placeholder="value" autocomplete="off" spellcheck="false" />
              <button id="add-btn">+</button>
            </div>
          </div>
          <div id="boxmodel-pane">
            <div class="pane-hdr" style="display:flex;align-items:center;cursor:pointer" id="boxmodel-hdr">
              <span style="flex:1">Box Model</span>
              <span id="boxmodel-arrow" style="font-size:9px;color:var(--text3);transition:transform 0.15s;display:inline-block;transform:rotate(90deg)">▶</span>
            </div>
            <div id="box-diagram" style="padding:10px"><div class="muted" style="padding:0">No element selected</div></div>
          </div>
        </div>
      </div>
...
<div id="phantom-pick-overlay"></div>
  ...
  PhantomDevTools.prototype._buildDOM = function () {
    ...
    self._styleRows  = q('#style-rows');
    self._addProp    = q('#add-prop');
    self._addVal     = q('#add-val');
    self._styleFilter= q('#style-filter');
  ...
  PhantomDevTools.prototype._bindEvents = function () {
    ...
    // Inspector divider drag (horizontal — resizes the right styles column)
    var div = s.querySelector('#insp-divider');
    div.addEventListener('mousedown', function (e) {
      e.preventDefault();
      var startX = e.clientX;
      var cols = self._inspBody.style.gridTemplateColumns || '1fr 3px 260px';
      var startW = parseInt(cols.split(' ')[2]) || 260;
      var onMove = function (me) {
        var delta = startX - me.clientX;
        var newW = Math.max(180, Math.min(520, startW + delta));
        self._inspBody.style.gridTemplateColumns = '1fr 3px ' + newW + 'px';
      };
    ...
    // Box model collapse toggle
    s.querySelector('#boxmodel-hdr').addEventListener('click', function () {
      var pane  = s.querySelector('#boxmodel-pane');
      var arrow = s.querySelector('#boxmodel-arrow');
      var collapsed = pane.classList.toggle('collapsed');
      arrow.style.transform = collapsed ? 'rotate(0deg)' : 'rotate(90deg)';
    });

    // Style filter
    self._styleFilter.addEventListener('input', function () {
      self._state.styleFilter = self._styleFilter.value.toLowerCase();
      self._applyStyleFilter();
    });

    // Add-rule row
    var doAdd = function () {
      var prop = self._addProp.value.trim();
      var val  = self._addVal.value.trim();
      if (!prop || !val || !self._state.selectedNode) return;
      try {
        self._state.selectedNode.style.setProperty(prop, val);
        self._addProp.value = '';
        self._addVal.value  = '';
        self._renderComputedStyles(self._state.selectedNode);
        self._renderBoxModel(self._state.selectedNode);
      } catch(_) {}
    };
    s.querySelector('#add-btn').addEventListener('click', doAdd);
    self._addVal.addEventListener('keydown', function (e) { if (e.key === 'Enter') { e.preventDefault(); doAdd(); } });
    self._addProp.addEventListener('keydown', function (e) { if (e.key === 'Enter') { e.preventDefault(); self._addVal.focus(); } });
  ...
  PhantomDevTools.prototype._ser = function (v, d) {
    ...
    if (Array.isArray(v)) {
      if (d > 3) return {t:'array', v:'Array('+v.length+')', len: v.length};
      var self = this;
      return {t:'array', v: v.slice(0,50).map(function (x) { return self._ser(x, d+1); }), len: v.length};
    }
    if (typeof v === 'object') {
      if (d > 3) return {t:'object', v:'[Object]'};
      try {
        var self = this, keys = Object.keys(v).slice(0,50), e = {};
  ...
  PhantomDevTools.prototype._renderEntry = function (entry, scroll) {
    ...
    body.appendChild(self._renderArgs(entry.args));
  ...
  PhantomDevTools.prototype._renderArgs = function (args) {
    ...
    (args || []).forEach(function (a, i) {
      ...
      frag.appendChild(self._makeValNode(a, 0));
    });
    return frag;
  };

  // Build a DOM node for a serialized value — supports click-to-expand for objects/arrays
  PhantomDevTools.prototype._makeValNode = function (v, depth) {
    var self = this;
    ...
    if (t === 'string')    {
      wrap.className += ' vs';
      // Show first 200 chars; if longer add expand link
      if (typeof val === 'string' && val.length > 200) {
        var short = document.createElement('span');
        short.textContent = val.slice(0, 200);
        var more = document.createElement('span');
        more.className = 'vnl';
        more.textContent = '…';
        more.style.cursor = 'pointer';
        more.title = 'Click to expand';
        more.addEventListener('click', function () {
          short.textContent = val;
          more.remove();
        });
    ...
    if (t === 'array') {
      if (!Array.isArray(val)) {
        // Collapsed stub — still show toggle to request expansion via REPL hint
        var tog = document.createElement('span');
        tog.className = 'ot-toggle';
        tog.textContent = '▶';
        var prev = document.createElement('span');
        prev.className = 'ot-preview vp';
        prev.textContent = 'Array(' + len + ')';
        wrap.appendChild(tog);
        wrap.appendChild(prev);
        return wrap;
      }

      var toggle = self._makeExpandToggle();
      var open = document.createElement('span');
      open.className = 'vp'; open.textContent = '[';
      var close = document.createElement('span');
      close.className = 'vp'; close.textContent = ']';

      // Inline preview (collapsed state)
      var preview = document.createElement('span');
      preview.className = 'ot-preview';
      var previewItems = val.slice(0, 4).map(function (item) {
        return self._makeValNode(item, depth + 1);
      });

      // Children container (expanded state)
      var children = document.createElement('div');
      children.className = 'ot-children';

      val.forEach(function (item, idx) {
        var row = document.createElement('div');
        row.className = 'ot-row';
        var key = document.createElement('span');
        key.className = 'ot-idx'; key.textContent = idx + ': ';
        row.appendChild(key);
        row.appendChild(self._makeValNode(item, depth + 1));
        children.appendChild(row);
      });
      if (len > val.length) {
        var ellip = document.createElement('div');
        ellip.className = 'ot-row vnl';
        ellip.textContent = '… ' + (len - val.length) + ' more items';
        children.appendChild(ellip);
      }

      // Build preview inline
      previewItems.forEach(function (n, i) {
        if (i > 0) { var c = document.createElement('span'); c.className='vp'; c.textContent=', '; preview.appendChild(c); }
        preview.appendChild(n);
      });
      if (len > 4) { var m = document.createElement('span'); m.className='vnl'; m.textContent=', …'; preview.appendChild(m); }

      var collapsed = document.createElement('span');
      collapsed.appendChild(open.cloneNode(true));
      collapsed.appendChild(preview);
      collapsed.appendChild(close.cloneNode(true));

      var expanded = document.createElement('span');
      var openE = open.cloneNode(true); expanded.appendChild(openE);
      expanded.appendChild(children);
      var closeE = close.cloneNode(true); expanded.appendChild(closeE);
      expanded.style.display = 'none';

      toggle.addEventListener('click', function (e) {
        e.stopPropagation();
        var isOpen = toggle.classList.toggle('o');
        children.classList.toggle('o', isOpen);
        collapsed.style.display = isOpen ? 'none' : '';
        expanded.style.display  = isOpen ? '' : 'none';
      });
      collapsed.addEventListener('click', function (e) { e.stopPropagation(); toggle.click(); });
    ...
    if (t === 'object') {
      if (typeof val !== 'object' || val === null) {
        wrap.className += ' vnl'; wrap.textContent = '[Object]'; return wrap;
      }
      ...
      var preview = document.createElement('span');
      preview.className = 'ot-preview';
      entries.slice(0, 3).forEach(function (kv, i) {
        if (i > 0) { var c = document.createElement('span'); c.className='vp'; c.textContent=', '; preview.appendChild(c); }
        var k = document.createElement('span'); k.className='vk'; k.textContent = kv[0];
        var col = document.createElement('span'); col.className='vp'; col.textContent=': ';
        preview.appendChild(k); preview.appendChild(col);
        preview.appendChild(self._makeValNode(kv[1], depth + 1));
      });
      if (len > 3) { var m = document.createElement('span'); m.className='vnl'; m.textContent=', …'; preview.appendChild(m); }
      ...
      entries.forEach(function (kv) {
        var row = document.createElement('div');
        row.className = 'ot-row';
        var k = document.createElement('span'); k.className='ot-key'; k.textContent = kv[0];
        var col = document.createElement('span'); col.className='ot-colon'; col.textContent=': ';
        row.appendChild(k); row.appendChild(col);
        row.appendChild(self._makeValNode(kv[1], depth + 1));
        children.appendChild(row);
      });
      if (len > entries.length) {
        var ellip = document.createElement('div');
        ellip.className = 'ot-row vnl';
        ellip.textContent = '… ' + (len - entries.length) + ' more keys';
        children.appendChild(ellip);
      }

      var expanded = document.createElement('span');
      expanded.appendChild(open.cloneNode(true));
      expanded.appendChild(children);
      expanded.appendChild(close.cloneNode(true));
      expanded.style.display = 'none';
  ...
  PhantomDevTools.prototype._makeExpandToggle = function () {
    var tog = document.createElement('span');
    tog.className = 'ot-toggle';
    tog.textContent = '▶';
    return tog;
  ...
  PhantomDevTools.prototype._renderComputedStyles = function (node) {
    ...
    if (!node || node.nodeType !== Node.ELEMENT_NODE) {
      self._styleRows.innerHTML = '<div class="muted">Select an element</div>';
      return;
    }
    var cs;
    try { cs = window.getComputedStyle(node); } catch(_) { return; }

    self._styleRows.innerHTML = '';
    var colorRx = /^(#|rgb|hsl|rgba|hsla|oklch)/i;
    var filterVal = (self._state.styleFilter || '').toLowerCase();

    CSS_PROPS.forEach(function (prop) {
      var computedVal = cs.getPropertyValue(prop);
      if (!computedVal || computedVal === 'none' || computedVal === 'normal') return;
      if (filterVal && prop.indexOf(filterVal) < 0 && computedVal.toLowerCase().indexOf(filterVal) < 0) return;

      // Inline override takes precedence for display
      var inlineVal = node.style.getPropertyValue(prop);
      var displayVal = inlineVal || computedVal;
      var isColor = colorRx.test(displayVal.trim());
      var isOverridden = !!inlineVal;

      var row = document.createElement('div');
      row.className = 'style-row' + (isOverridden ? '' : '');
      row.dataset.prop = prop;

      // Property name span (non-editable label, click to focus value)
      var propSpan = document.createElement('span');
      propSpan.className = 'sp';
      propSpan.textContent = prop;
      row.appendChild(propSpan);

      // Value span — contenteditable
      var valSpan = document.createElement('span');
      valSpan.className = 'sv' + (isColor ? ' swatch' : '');
      if (isColor) valSpan.style.setProperty('--_sw', displayVal);
      ...
      valSpan.addEventListener('focus', function () {
        ...
        var sel = self._shadow.getSelection ? self._shadow.getSelection() : window.getSelection();
      ...
      valSpan.addEventListener('keydown', function (e) {
        ...
        if (e.key === 'Escape') {
          valSpan.textContent = inlineVal || computedVal;
          valSpan.classList.remove('invalid');
          valSpan.blur();
        }
        if (e.key === 'Tab') {
          e.preventDefault();
          // Move to next row's value
          var rows = Array.from(self._styleRows.querySelectorAll('.sv'));
          var idx = rows.indexOf(valSpan);
          if (idx >= 0 && idx < rows.length - 1) rows[idx+1].focus();
        }
      });

      valSpan.addEventListener('input', function () {
        var newVal = valSpan.textContent.trim();
        // Live preview
        try {
          node.style.setProperty(prop, newVal);
          valSpan.classList.remove('invalid');
          if (colorRx.test(newVal)) {
            valSpan.classList.add('swatch');
            valSpan.style.setProperty('--_sw', newVal);
          } else {
            valSpan.classList.remove('swatch');
          }
        } catch(_) {
          valSpan.classList.add('invalid');
        }
      });
      ...
      valSpan.addEventListener('blur', function () {
        var newVal = valSpan.textContent.trim();
        if (!newVal) {
          // Remove the property if cleared
          node.style.removeProperty(prop);
          valSpan.textContent = computedVal;
          valSpan.classList.remove('invalid');
        } else {
          try {
            node.style.setProperty(prop, newVal);
        ...
        // Refresh box model after editing geometry props
        var geoProps = ['width','height','margin','margin-top','margin-right','margin-bottom','margin-left','padding','padding-top','padding-right','padding-bottom','padding-left','border-width','border-top-width','border-right-width','border-bottom-width','border-left-width'];
        if (geoProps.indexOf(prop) > -1) self._renderBoxModel(node);
      });
      ...
      del.addEventListener('click', function () {
        node.style.removeProperty(prop);
        // Re-render to show computed value without override
        self._renderComputedStyles(node);
        self._renderBoxModel(node);
      });
      row.appendChild(del);

      self._styleRows.appendChild(row);
    });

    if (!self._styleRows.children.length) {
      self._styleRows.innerHTML = '<div class="muted">No matching properties</div>';
    }
  };

  PhantomDevTools.prototype._applyStyleFilter = function () {
    var self = this;
    if (self._state.selectedNode) self._renderComputedStyles(self._state.selectedNode);
  };
