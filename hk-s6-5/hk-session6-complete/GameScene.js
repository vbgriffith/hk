...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this._audio     = AUDIO_ENGINE;   // Web Audio procedural engine
  ...
  _loadRoom(roomKey, spawnPoint = 'default') {
    ...
    // ── Music ─────────────────────────────────────────────────────────────
    // Play room music via Web Audio engine
  ...
  _clearRoom() {
    // ── Cancel any in-progress dialogue (prevents stale timer callbacks) ─────
    this._dialogue?.cancelTyping?.();
    if (this._dialogue?.isVisible) {
      // Fully reset dialogue state before transition
      this._dialogue._visible = false;
      this._dialogue._callback = null;
      this._dialogue._typing = false;
      this._dialogue._index = 0;
      this._dialogue._lines = [];
      ...
      // Resume physics (dialogue.show() pauses it)
  ...
  _transitionRoom(destRoom, destSpawn) {
    ...
    // Block exit while boss is active
  ...
  onPlayerDeath() {
    ...
    this._save.deaths++;
    // Save with geo=0 (already zeroed in knight._die) and shade data
    SaveSystem.save(this._buildSaveData());
  ...
  _buildSaveData() {
    ...
    // Build a clean snapshot — do NOT mutate this._save directly
    const snap = Object.assign({}, this._save, kData);
    
    // Ensure critical fields are always valid objects/arrays
    if (!snap.flags || typeof snap.flags !== 'object') {
      snap.flags = (this._save && this._save.flags && typeof this._save.flags === 'object') 
        ? Object.assign({}, this._save.flags) 
        : {};
    }
    if (!snap.abilities || typeof snap.abilities !== 'object') {
      snap.abilities = (this._save && this._save.abilities && typeof this._save.abilities === 'object')
    ...
    if (!Array.isArray(snap.visitedRooms)) {
      snap.visitedRooms = Array.isArray(this._save?.visitedRooms)
        ? [...this._save.visitedRooms]
        : [];
    }
    if (!Array.isArray(snap.ownedCharms)) {
      snap.ownedCharms = Array.isArray(this._save?.ownedCharms)
        ? [...this._save.ownedCharms]
        : [];
    }
    if (!Array.isArray(snap.charms)) {
      snap.charms = Array.isArray(this._save?.charms)
        ? [...this._save.charms]
        : [];
    }
    return snap;
...
GameScene.prototype.registerNailHit = function(nailBox) {
  ...
  for (const boss of this._bosses ?? []) {
    ...
    if (!boss.sprite?.active) continue;
    this.physics.add.overlap(nailBox, boss.sprite, () => {
      if (!nailBox.active) return;
      if (!boss.alive) return;
      const nailMult = (typeof this.knight?._nailDmgMult === 'number') ? this.knight._nailDmgMult : 1;
      boss.onHit(nailBox.dmg * nailMult, this.knight);
      this.knight?.gainSoul?.();
      this._hud?.update();
      if (nailBox.dir === 'down') {
        this.knight?.body?.setVelocityY(C.KNIGHT_JUMP_VEL * 0.7);
