...
const Generator = (() => {
  ...
  function generateActs(base, config, locations, twists) {
    ...
    for (var i = 0; i < config.acts; i++) {
      ...
      acts.push({
        ...
        twist:        actTwist,
        socialEncounters: generateSocialEncounters({ number: i+1, title: pick(titles), levelStart: lvl.start, levelEnd: lvl.end }, config, base.defaultVillain, null),
      });
    }
    return acts;
  }
  ...
  function generateSocialEncounters(act, config, villain, npcs) {
    if (typeof SOCIAL_DATA === 'undefined') return null;
    var partyLevel = Math.round((act.levelStart + act.levelEnd) / 2);
    var templates  = SOCIAL_DATA.socialEncounterTemplates;
    var matrices   = SOCIAL_DATA.npcPersonalityMatrices;
    var numScenes  = Math.min(2, config.sideQuests || 2);
    var scenes     = [];

    // 1. Act-specific influence encounter (tied to act's primary NPC)
    var template  = pick(templates);
    var matrix    = pick(matrices);
    var npcLevel  = clamp(partyLevel + rand(-2,2), 1, 20);
    var baseDC    = SOCIAL_DATA.influenceDCByLevel(npcLevel);

    // Build attitude track for this NPC
    var startIdx  = rand(0, 2); // start Hostile, Unfriendly, or Indifferent
    var targetIdx = rand(3, 4); // need Friendly or Helpful

    // Assign DCs to checks based on level
    var checks = (template.skillChecks || []).map(function(sc) {
      return {
        skill:   sc.skill,
        dc:      baseDC + sc.dc_mod,
        purpose: sc.purpose,
        rules:   SOCIAL_DATA.socialSkills[sc.skill] || null,
      };
    });

    // Generate a named NPC for this scene if none in campaign
    var npcPool = npcs && npcs.length ? npcs : [{ name:'Key Contact', role:'Influential Figure', personality:'Guarded but ultimately reasonable' }];
    var sceneNPC = pick(npcPool);
    ...
    scenes.push({
      ...
      complication: pick(SOCIAL_DATA.complications),
      npc:          sceneNPC,
      npcMatrix:    matrix,
      startAttitude:SOCIAL_DATA.attitudeTrack[startIdx],
      targetAttitude:SOCIAL_DATA.attitudeTrack[targetIdx],
      stepsNeeded:  targetIdx - startIdx,
      checks:       checks,
      openingLine:  pick(SOCIAL_DATA.openingLines),
      conditionLine:pick(SOCIAL_DATA.conditionLines),
      refusalLine:  pick(SOCIAL_DATA.refusalLines),
      revealLine:   pick(SOCIAL_DATA.revealLines),
      villainTie:   generateVillainTie(villain, matrix),
    });

    // 2. Skill challenge for this act
    var challenge = pick(SOCIAL_DATA.skillChallenges);
    scenes.push({
      type:        'SkillChallenge',
      name:        challenge.name,
      ...
      skills:      challenge.skills.map(function(s) {
        return { skill: s, dc: baseDC, rules: SOCIAL_DATA.socialSkills[s] || null };
      }),
      stakes:      challenge.stakes,
      complication:pick(SOCIAL_DATA.complications),
      actTie:      'Resolving this scene advances ' + act.title + ' — success unlocks the next major location.',
    });

    // 3. Reputation snapshot for this act's factions
    var repSnapshot = generateReputationSnapshot(act, config, null);


    return { scenes: scenes, reputationSnapshot: repSnapshot };
  }

  function generateReputationSnapshot(act, config, factions) {
    if (typeof SOCIAL_DATA === 'undefined') return null;
    var posEvents = SOCIAL_DATA.reputationEvents.positive;
    var negEvents = SOCIAL_DATA.reputationEvents.negative;
    // Use passed faction names, or fall back to base component faction names, or generic
    var factionNames = (factions && factions.map(function(f){ return f.name; })) ||
      (typeof COMPONENTS !== 'undefined' && COMPONENTS.factions ?
        pickN(COMPONENTS.factions, 3).map(function(f){ return f.name; }) :
        ['City Watch','Merchant Guild','The Underground','Noble Court']);
    var numFactions = Math.min(2, factionNames.length);
    var snapshots = [];
    for (var i = 0; i < numFactions; i++) {
      var gains = pickN(posEvents, 2).map(function(e) {
        return { points: e.points, event: e.event };
      });
      var losses = pickN(negEvents, 2).map(function(e) {
        return { points: e.points, event: e.event };
      });
      snapshots.push({
        faction: factionNames[i] || ('Faction ' + (i+1)),
        note:    'Act ' + act.number + ' — reputation stakes for this faction',
        gains:   gains,
        losses:  losses,
      });
    }
    ...
    function generateVillainTie(villain, matrix) {
    var ties = [
      "The villain has already approached this NPC -- they refused, but are shaken.",
      "This NPC has information about the villain's past that even the villain doesn't know exists.",
      "The villain holds leverage over this NPC's family or organization.",
      "This NPC used to trust the villain completely -- that trust was destroyed by a specific incident.",
      "The NPC is secretly testing whether the heroes are truly different from the villain.",
      "The villain sent this NPC as a probe -- if heroes gain their trust, the NPC reports back."
    ];
    return pick(ties);
  }
  ...
  function generateRewards(config, acts) {
    if (typeof LOOT_DATA !== 'undefined') {
      var theme = config.theme || 'generic';
      return acts.map(function(act) {
        return {
          act:   act.number,
          hoard: LOOT_DATA.generateActHoard(act.levelStart, act.levelEnd, config.partySize || 4, theme),
        };
      });
    }
  ...
  async function forge(overrides) {
    ...
    // Patch reputation snapshots with real faction names now that factions are known
    if (typeof SOCIAL_DATA !== 'undefined') {
      acts.forEach(function(act) {
        if (act.socialEncounters) {
          act.socialEncounters.reputationSnapshot = generateReputationSnapshot(act, config, factions);
        }
      });
    }
    var rewards      = generateRewards(config, acts);
    var sessionTables = (typeof RANDOM_TABLES !== 'undefined')
      ? RANDOM_TABLES.generateSessionTables({
          theme: base.theme || config.theme,
          environment: locations[0] || '',
          title: base.title || '',
          season: config.season || 'temperate_spring',
        })
      : null;
    ...
    _current = {
      ...
      acts: acts, rewards: rewards, sessionTables: sessionTables,
